...
#define UNUSED(...) internal::unused(__VA_ARGS__)

#define CHECK(pred, ...) \
  do { \
    if (!(pred)) { \
      throw std::runtime_error(internal::sstreamf() \
                  << "assertion `" STRINGIZE(pred) \
                  << "` failed, " << internal::stringf(__VA_ARGS__)); \
    } \
  } while (false)
  ...
  #define DBGCHECK(pred, ...) UNUSED(pred)
  #define DBG(level, ...)
#else
  #define ABORT(...) \
    do { \
  ...
  #define DBGCHECK CHECK
  #define DBG(level, ...) internal::dbprint(level, __VA_ARGS__)
#endif

#define DEF_SFINAE_CHECK(type_name, predicate) \
...
#define TODO() ABORT("Not yet implemented");
