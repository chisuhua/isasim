...
SimtStack::SimtStack(unsigned wid, unsigned warpSize, libcuda::gpgpu_context *ctx) {
  m_gpgpu_ctx = ctx;
  m_warp_id = wid;
...
void SimtStack::reset() { m_stack.clear(); }

void SimtStack::launch(addr_t start_pc, const simt_mask_t &active_mask) {
  reset();
...
void SimtStack::resume(char *fname) {
  reset();
...
const simt_mask_t &SimtStack::get_active_mask() const {
  assert(m_stack.size() > 0);
  return m_stack.back().m_active_mask;
}

void SimtStack::get_pdom_stack_top_info(unsigned *pc, unsigned *rpc) const {
  assert(m_stack.size() > 0);
  *pc = m_stack.back().m_pc;
  *rpc = m_stack.back().m_recvg_pc;
}

unsigned SimtStack::get_rp() const {
  assert(m_stack.size() > 0);
...
void SimtStack::print_checkpoint(FILE *fout) const {
  for (unsigned k = 0; k < m_stack.size(); k++) {
    SimtStack_entry stack_entry = m_stack[k];
...
void SimtStack::update(simt_mask_t &thread_done, addr_vector_t &next_pc,
