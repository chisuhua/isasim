...
void INST::Decode(uint64_t _opcode) {
    ...
    dsrc0_Decode(this, OPCODE.imm_, OPCODE.dsrc0_, OPCODE.src0, m_size, OPCODE.ext.e1_.imm, OPCODE.ext.e1_.src0, src1_reg_range, Operand::SRC1);
    if (m_size == 8) {
...
void INST::V_MULLO_I32_I24(WarpState *w, uint32_t lane_id)
{
    // Load operands from registers or as a literal constant.
    Register s0 = operands_[Operand::SRC0]->getValue(lane_id);
    Register s1 = operands_[Operand::SRC1]->getValue(lane_id);
...
void INST::V_ADD_I32(WarpState *w, uint32_t lane_id)
{
    ...
    sum.as_uint = funit_->add(s0.as_int, s1.as_int, carry.as_uint, 0);
    // static int run_times {0};
    // run_times++;

    // funit_->adder32_sim->toText(internal::stringf("adder%d.log", run_times));
    // funit_->adder32_sim->toVCD(internal::stringf("adder%d.vcd", run_times));
...
// D.u = S0.u * S1.u.
void INST::V_MULLO_U32(WarpState *item, uint32_t lane_id)
{
	Register s0;
	Register s1;
	Register result;
...
// D.u = (S0.u * S1.u)>>32
void INST::V_MULHI_U32(WarpState *item, uint32_t lane_id)
{
    Register s0 = operands_[Operand::SRC0]->getValue(lane_id);
    Register s1 = operands_[Operand::SRC1]->getValue(lane_id);
	Register result;
...
// D.i = S0.i * S1.i.
void INST::V_MULLO_I32(WarpState *item, uint32_t lane_id)
{
...
// D.u = (S0.u * S1.u)>>32
void INST::V_MULHI_I32(WarpState *item, uint32_t lane_id)
{
    Register s0 = operands_[Operand::SRC0]->getValue(lane_id);
    Register s1 = operands_[Operand::SRC1]->getValue(lane_id);
	Register result;
...
*/
	...
	// Calculate the product and shift right.
	result.as_uint = (unsigned)
		(((unsigned long long)s0.as_int *
		(unsigned long long)s1.as_int) >> 32);
